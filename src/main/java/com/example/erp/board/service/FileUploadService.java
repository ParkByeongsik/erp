...
public class FileUploadService {
    ...
    //파일명을 전달받아서 업로드 폴더경로와 파일명을 연결해서 실제 어느위치에 어떤 파일로 저장될지 full path를 만들어서 리턴하는 메소드
    ...
    public List<BoardFileDTO> uploadFiles(List<MultipartFile> multipartFiles) throws IOException {
        ...
        for (MultipartFile multipartFile : multipartFiles) {
            if (!multipartFile.isEmpty()) { //파일의 내용이 empty상태인지 평가
                ...
                fileDTOList.add(new BoardFileDTO(UUID.randomUUID().toString(),
    ...
    private String createStoreFilename(String originalFilename){
        //확장자를 추출
        int position = originalFilename.lastIndexOf(".");//확장자 위치구하기
